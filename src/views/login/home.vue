<template>
    <div class="login">
           <form action="" @submit.prevent="onsubmit">
               ...
               <input type="text" placeholder="邮箱/手机号码/小米id" v-model="customers.username">
               <div style="color: red">{{errors.email}}</div>
               <div class="eye">
                   <input :type="num" placeholder="密码" v-model="customers.password">
               ...
               <input type="submit" value="登入" style="text-align: center; background-color:#ff6700
               ;border-radius: 10px;height: 40px;border-bottom: 0;color: white;font-size: 15px">
               <input type="button" @click="register" value="手机短信登入/注册" style="text-align: center; background-color:#ff6700
               ;border-radius: 10px;height: 40px;border-bottom: 0;color: white;font-size: 15px">
           </form>
        <div class="reverse">
...
<script>
    export default {
        ...
        methods:{
            ...
            onsubmit(){
                    this.axios.post(`http://localhost:8000/oauth/token`,this.customers).then(res=>{
                        console.log(res)
